...
class Invoice:
    def __init__(self, index, client, product, date_string, currency,
                 payment_option):
        ...
        self.data = {
            ...
            'total': {
                ...
                'tax': self.product,
                'incl_tax':
                self.product.total_tax_excl + self.product.total_tax
            }
        }
    ...
    def parse_date(self):
        ...
        self.data['payment_date'] = date + datetime.timedelta(
            days=options['payment_date_delay'])
    ...
    def get_payment_details(self, option):
        ...
        if not option_string in options['payment_options'].keys():
            logging.warning(
                'Unknown payment option "{!s}"'.format(option_string))
            return ''
    ...
    def get_invoice_html(self, invoice_data):
        ...
        for index, identifier in self.regex_matches:
            ...
            try:
                ...
                logging.warning(
                    'Could not find matching value for {!s}'.format(
                        identifier))
            if identifier in [
                    'product_unit_price', 'product_total_tax_excl',
                    'total_discount', 'total_excl_tax', 'total_tax',
                    'total_incl_tax'
            ]:
                replace_value = str(
                    replace_value) + invoice_data['invoice']['currency']

            html[index] = html[index].replace(string_template,
                                              str(replace_value), 1)
        return html
